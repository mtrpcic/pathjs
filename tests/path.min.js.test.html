...
<html>
	<head>
		...
		function define_routes(){
			...
			Path.map("#I").to(function() {
				var output = [];
				if(this.params.hasOwnProperty("debug")) {
					output.push("debug=" + this.params["debug"]);
				}
				if(this.params.hasOwnProperty("id")) {
					output.push("id=" + this.params["id"]);
				}
				update("I(" + output.join(", ") + ")");
			});
			...
			Path.root("#F");
			Path.listen();
			timer = setInterval(run_route, 200);
		}
		</script>
